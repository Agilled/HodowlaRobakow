...
{
	int height = Map::GetInstace().getHeight();
	int width = Map::GetInstace().getWidth();
	...
	for (int y = 0; y < height; y++)
	{
		...
		{
			if (Map::GetInstace().tileMap[y][x].interactable)
			{
...
int Food::isEating(Worm worm, float cooldown)
{
	Vector2f corner(worm.getX(), worm.getY());
	Vector2f size(worm.getGlobalBounds().width, worm.getGlobalBounds().height);
	...
	for (int i = imin; i <= imax && i < foods.size() ; i++)
	{
		for (int j = jmin; j <= jmax && j < foods[i].size(); j++)
		{
			if (foods[i][j].active == true)
			{
				if (worm.getGlobalBounds().intersects(foods[i][j].shape.getGlobalBounds()))
				{
					eaten(i, j, cooldown);
					eat++;
				}
